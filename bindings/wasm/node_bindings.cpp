...
// Prefix-scoped enumeration (mirrors bindings/c/whiteout_casc.cpp's
// listFilesPrefix), exposed here rather than via the autogenerated
// casc_bindings.cpp to avoid a duplicate `CascStorage` registration
// (see the CMakeLists.txt comment in bindings/wasm).
val cascListFilesPrefix(const storages::casc::Storage& self,
                        const std::string& prefix) {
    val out = val::array();
    int i = 0;
    self.enumerate(prefix + "/*", [&](const storages::casc::EnumerateEntry& entry) {
        val e = val::object();
        e.set("path", entry.path);
        e.set("fileSize", static_cast<double>(entry.fileSize));
        out.set(i++, e);
        return true;
    });
    return out;
...
EMSCRIPTEN_BINDINGS(whiteout_node) {
    ...
    class_<storages::casc::Storage>("CascStorage")
        ...
        .function("listFilesPrefix", &cascListFilesPrefix);
#endif
}
